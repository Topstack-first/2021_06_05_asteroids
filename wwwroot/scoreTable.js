...
const appendScores = (singleScore, singleScoreIndex) => {
    ...
    scoreboardTableBodyRow.append(scoreRanking, usernameData, scoreData) // Append all 3 cells to the table row
    scoreboardTable.append(scoreboardTableBodyRow) // Append the current row to the scoreboard table body
}
const getScores = () => {
    fetch('api/players/history') // Fetch for all scores. The response is an array of objects that is sorted in decreasing order
        .then(res => res.json())
        .then(scores => {
